 ...
 * 
 * @example
 * Manager.methods.pushData('users/messages', someMessage, anotherMessage)
 */
const pushData = (reference, ...values) => getData(reference, [])
    .then(oldValues => setData(reference, [...oldValues, ...values]))

/** 
 * Increment a data from the database by a reference
 * @param {String} reference Reference to the database
 * @param {Number} [values=1] Amount to be incremented
 * @returns {Promise<Number>}
 * 
 * @example
 * Manager.methods.incremenetData('user/someID/likes', 1)
 * .then(newAmount => console.log(`Now this user has ${newAmount} likes!`))
 */
const incrementData = (reference, amount = 1) => getData(reference, 0)
    .then(oldNumber => {
...
/** 
 * Decrement a data from the database by a reference
 * @param {String} reference Reference to the database
 * @param {Number} [values=1] Amount to be incremented
 * @returns {Promise<Number>}
 * 
 ...
 * .then(newAmount => console.log(`Now this user has ${newAmount} likes!`))
 */
const decrementData = (reference, amount = 1) => getData(reference, 0)
    .then(oldNumber => {
        const newAmount = +oldNumber - amount
